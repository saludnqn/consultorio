...
namespace Consultorio.Turnos
{
    ...
    public partial class DiagnosticoEdit : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("TurnoNuevoDefault.aspx?Desde=DiagnosticoEdit.aspx&idTurnoAcompaniante=" + lblIdTurno.Text, false);
        }
        ...
        protected void gvTurnosAcompaniante_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Eliminar")
            {
                ...
                if (!tienediagnostico(t))
                {
                    ...
                    Response.Redirect("DiagnosticoEdit.aspx?idAgenda=" + tprincipal.IdAgenda.ToString() + "&idTurno=" + tprincipal.IdTurno.ToString(), false);
                }
        ...
        protected void btnVolverPacientePrincipal_Click(object sender, EventArgs e)
        {
            ...
            ConTurno t = new ConTurno(int.Parse(Request["idTurno"].ToString()));
            Response.Redirect("DiagnosticoEdit.aspx?idAgenda=" + t.IdAgenda.ToString() + "&idTurno=" + t.IdTurnoAcompaniante.ToString(), false);
        ...
        protected void gvHistorial_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.Cells.Count > 1)
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    ConAgenda agenda = new ConAgenda(int.Parse(gvHistorial.DataKeys[e.Row.RowIndex].Value.ToString()));
